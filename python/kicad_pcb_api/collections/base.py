"""
Base collection class for unified PCB element management.

Provides common functionality for indexing, searching, and managing
collections of PCB elements with performance optimization.

Based on kicad-sch-api's IndexedCollection pattern.
"""
...
class IndexedCollection(Generic[T], ABC):
    """
    Base class for all PCB element collections with automatic indexing.

    Provides unified functionality for:
    - UUID-based fast lookups (O(1))
    ...
    - _build_additional_indexes(): Build type-specific indexes
    """
    ...
    def __init__(self, items: Optional[List[T]] = None):
        ...
        if items:
            ...
            # Mark as clean since this is initialization, not modification
            self._modified = False
    ...
    def filter(self, **criteria) -> List[T]:
        ...
        Example:
            collection.filter(layer='F.Cu', net='GND')
        """
    ...
    def __contains__(self, item: Union[str, T]) -> bool:
        ...
        Args:
            item: UUID string or item instance

        Returns:
            True if item/UUID is in collection
        """
    ...
    def _rebuild_indexes(self) -> None:
        ...
        # Rebuild UUID index
        self._uuid_index = {
            self._get_item_uuid(item): i for i, item in enumerate(self._items)
        }
